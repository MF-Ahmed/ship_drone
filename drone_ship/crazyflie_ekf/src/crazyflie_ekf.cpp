...
: node_(node)
{
  ...
  node_->declare_parameter("base_frame", "crazyflie/base_footprint");
  ...
  RCLCPP_INFO(node_->get_logger(), "Crazyflie EKF node initialized");
...
void CrazyflieEKF::gpsCallback(const sensor_msgs::msg::NavSatFix::SharedPtr msg)
{
  ...
  RCLCPP_INFO_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000,
  "EKF Estimate: [%.2f, %.2f, %.2f]  Vel: [%.2f, %.2f, %.2f]",
  ...
  odom_msg.header.frame_id = "crazyflie/odom";
